...
namespace LibraryManagementSystem
{
    public partial class AddBooks : UserControl
    {
        ...
        private void addBooks_addBtn_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Моля попълнете всички празни полета.", "Грешка", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else
            {
                if(connect.State == ConnectionState.Closed)
                {
                    try
                    {
                        ...
                        string path = Path.Combine(@"\Books_Directory\" +
                        ...
                        using(SqlCommand cmd = new SqlCommand(insertData, connect))
                        {
                            ...
                            MessageBox.Show("Добавена успешно!", "Информация", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void addBooks_updateBtn_Click(object sender, EventArgs e)
        {
            if (addBooks_picture.Image == null
                ...
                || addBooks_picture.Image == null)
            {
                MessageBox.Show("Моля първо изберете книга", "Грешка", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else
            {
                if(connect.State != ConnectionState.Open)
                {
                    DialogResult check = MessageBox.Show("Сигурни ли сте, че искате да ПРОМЕНИТЕ Book ID: "
                        + bookID + "?", "Потвърждение", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                    ...
                    if(check == DialogResult.Yes)
                    {
                        try
                        {
                            ...
                            using (SqlCommand cmd = new SqlCommand(updateData, connect))
                            {
                                ...
                                MessageBox.Show("Променено успешно", "Информация", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    ...
                    {
                        MessageBox.Show("Отменено.", "Информация", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
        ...
        private void addBooks_deleteBtn_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                if (connect.State != ConnectionState.Open)
                {
                    DialogResult check = MessageBox.Show("Сигурни ли сте, че искате да ИЗТРИЕТЕ Book ID:"
                        + bookID + "?", "Потвърждение", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                    ...
                    if (check == DialogResult.Yes)
                    {
                        try
                        {
                            ...
                            using (SqlCommand cmd = new SqlCommand(updateData, connect))
                            {
                                ...
                                MessageBox.Show("Изтриване успешно!", "Информация", MessageBoxButtons.OK, MessageBoxIcon.Information);
